...
\blandscape
```
```{r Figure1, echo=FALSE, out.height = '70%', out.width = '80%', fig.align='center'}
...
*Figure 2. Examples of brain imaging visualization make in using different Python packages.* A) Voxel-based statistical map threshold and overlaid over a T1-weighted template data, with a single axial slice shown. Made using the `plotting.plot_stat_map` function from `nilearn`. B) Voxel-level cortical parcellation overlaid on T1-weighted MRI data shown on axial slices. Made using `plotting.plot_roi` function from `nilearn`. C) Medial and lateral views of an cortex-wide ROI atlas, displayed on an inflated template surface where a random value has been assigned to each ROI. Made using `Plot` function from `surfplot`. D) Lateral and medial views of vertex-level data displayed in a 3D rendering on an individual's white matter surface. Made using plotting.view_surf function in from `nilearn`. 3D rendering of 16 subcortical structures from the the Deiskan-Killiany atlas, where a random value has been assigned to each region. Made using the `plot_subcortical` function from the `ENIGMA TOOLBOX`.A weighted and undirected graph plotted on front, right, and top views of a schematic outline of a brain in standardized coordinate space. Made using plotting.plot_connectome function in `nilearn`.All code used to generate the contents of each panel, and compile this figure are provided in an accompanying [online repository](https://github.com/sidchop/RepoNeuroVis).
