...
The following four sections provide brief examples of packages and functions available in R and Python for visualizing voxel, vertex, ROI and edge-level data. This is not an exhaustive list of packages available for visualizing brain data in R and Python, rather, the following sections aim to give the reader a sense of the available options, and an entry point to using them. A didactic introduction to the R based tools is provided in an accompanying [collection of beginner friendly R-Notebooks](https://github.com/sidchop/neuRo-vis-BrainHack-2021).
...
\blandscape
```
```{r Figure2 , echo=FALSE, out.height = '80%', out.width = '80%', fig.align='center'}
...
patchwork + plot_annotation(tag_levels = 'A',
                            title = 'Examples of brain visualizations made in Pythoin') &
    theme(plot.tag = element_text(face = 'bold'),
          plot.title = element_text(face="bold"))
